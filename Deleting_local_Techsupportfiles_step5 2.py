...
def Deleting_local_Techsupport_files(APIC,User,Password):
    with open("techcollection/Deleting_Local_Techsupportfiles.txt", "w") as f:
        ...
        cmds = ["sh version | grep leaf |  awk '{ print $4 }'"]
        for cmd in cmds:
        ...
        # # Collectfiles_nodes_more30days = ssh234.send_command("find /data/techsupport/ -mtime +30 -ls | grep .gz")
        # print(Collectfiles_more30days)
        # # print(Collectfiles_nodes_more30days)
        # f.write(f"{Collectfiles_more30days}")
        # print("#" * 50)
        # f.write("#" * 50)
    with open("techcollection/Deleting_Local_Techsupportfiles.txt", "r") as f:
        Leaf_switches = f.read().split()
        print(Leaf_switches)
        print(type(Leaf_switches))
        for nodes in Leaf_switches:
            nodes = nodes.strip()
            devices1234 = {
                    "device_type": "cisco_nxos",
                    "host": nodes,
        ...
        ssh234 = ConnectHandler(**devices1234)
        Collectfiles_nodes_more30days = ssh234.send_command("find /data/techsupport/ -mtime +30 -ls | grep .gz")
        print(Collectfiles_nodes_more30days)
        print("#" * 50)
        f.write("#" * 50)
